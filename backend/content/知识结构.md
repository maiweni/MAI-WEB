...
### 2、无限制qwen image edit实现16G显存生图，并使用4step
...
- 工作流使用
  ...
  - 工作流使用comfyui自带的qwen image edit 2509的工作流
  - 模型使用：https://huggingface.co/Phil2Sat/Qwen-Image-Edit-Rapid-AIO-GGUF

- 注意事项：

  - 原生工作流默认使用的是safetensors格式的模型，因此，这里使用的gguf格式的工作流没有办法加载，因此需要更换节点，将对应的模型加载节点，编码节点，vae加载节点都替换成加载gguf的节点。
  - 下载的Qwen2.5-VL-7B-Instruct-abliterated的权重，需要配合mmproj模型使用，mmproj模型下载链接与Qwen2.5-VL-7B-Instruct-abliterated在一起，在使用的时候需要将两者放到一个文件夹中，否则会报错。
  - readme中提到的需要更改qwen_node.py，这里暂时没有更换，但是并未发生报错。
...
注意：如果这里修改了OPENAI_BASE_URL的话，那么codex等需要使用OPENAI_BASE_URL的工具就会找错工具
```
